//+------------------------------------------------------------------+
//|  ULTIMATE v6 GOD MODE EA — ICT/SMC Confluence                   |
//|  Converted from Pine Script by Onwun                            |
//|  MQL5 Expert Advisor — Full Auto Trading                        |
//+------------------------------------------------------------------+
#property copyright "ULTIMATE GOD MODE EA v6"
...
input int    i_strongThr      = 85;     // Strong Signal Threshold (60-100)
input int    i_modThr         = 70;     // Moderate Signal Threshold (50-84)
input int    i_godThr         = 95;     // God Mode Threshold (90-100)

// MTF
input group "🕐 Multi-Timeframe"
input ENUM_TIMEFRAMES i_tf1   = PERIOD_H1;   // HTF 1
input ENUM_TIMEFRAMES i_tf2   = PERIOD_H4;   // HTF 2
input ENUM_TIMEFRAMES i_tf3   = PERIOD_D1;   // HTF 3
input ENUM_TIMEFRAMES i_tf4   = PERIOD_W1;   // HTF 4
input ENUM_TIMEFRAMES i_tf5   = PERIOD_MN1;  // HTF 5
input bool   i_useTF1         = true;   // Use HTF 1
input bool   i_useTF2         = true;   // Use HTF 2
input bool   i_useTF3         = true;   // Use HTF 3
input bool   i_useTF4         = false;  // Use HTF 4
input bool   i_useTF5         = false;  // Use HTF 5

// ICT/SMC
input group "🏦 ICT / SMC Modules"
input bool   i_useOB          = true;   // Order Blocks
input bool   i_useFVG         = true;   // Fair Value Gaps
input bool   i_useBreaker     = true;   // Breaker Blocks
input bool   i_useUnicorn     = true;   // Unicorn Setup
input bool   i_useLiqSweep    = true;   // Liquidity Sweep + BOS
input bool   i_useOTE         = true;   // OTE Fib Retracement
input bool   i_useBPR         = true;   // Balanced Price Range
input bool   i_useInducement  = true;   // Inducement + Displacement
input bool   i_useMSS         = true;   // MSS / CHoCH
input bool   i_useKZ          = true;   // Kill Zones
input bool   i_useHTFOB       = true;   // Weekly/Monthly OB

// Classical TA
input group "📈 Classical TA Modules"
input bool   i_useEngulf      = true;   // Engulfing Candles
input bool   i_useHammer      = true;   // Hammer / Shooting Star
input bool   i_useStar        = true;   // Morning / Evening Star
input bool   i_useDoji        = true;   // Doji at S/R
input bool   i_useDivReg      = true;   // Regular Divergence
input bool   i_useDivHid      = true;   // Hidden Divergence
input bool   i_useBBSqz       = true;   // BB Squeeze Breakout
input bool   i_useST          = true;   // Supertrend Flip @ OB
input bool   i_useEMA         = true;   // EMA Ribbon Pullback
input bool   i_useIchi        = true;   // Ichimoku + TK Cross

// Volume
input group "📦 Volume / Footprint"
input bool   i_useVolDelta    = true;   // Volume Delta at Zones
input bool   i_useFootPOC     = true;   // POC Rejection

// Trade Management
input group "💰 Trade Management"
input double i_slATRMult      = 1.5;    // SL ATR Multiplier
input double i_tp1ATRMult     = 2.0;    // TP1 ATR Multiplier
input double i_tp2ATRMult     = 3.0;    // TP2 ATR Multiplier
input double i_tp3ATRMult     = 4.0;    // TP3 ATR Multiplier
input bool   i_useTrailStop   = true;   // Use Trailing Stop
input double i_trailATRMult   = 1.0;    // Trail Stop ATR Multiplier
input bool   i_useBreakEven   = true;   // Use Break Even
input double i_beATRMult      = 1.5;    // Break Even at ATR Multiplier

// Sessions
input group "🕐 Session Filters"
input bool   i_sessFilter     = false;  // Filter by Kill Zone Sessions
input int    i_asianStart     = 0;      // Asian Session Start Hour (GMT)
input int    i_asianEnd       = 2;      // Asian Session End Hour (GMT)
input int    i_londonStart    = 7;      // London Session Start Hour (GMT)
input int    i_londonEnd      = 10;     // London Session End Hour (GMT)
input int    i_nyStart        = 13;     // New York Session Start Hour (GMT)
input int    i_nyEnd          = 17;     // New York Session End Hour (GMT)

// Pivot
input int    i_pivLen         = 5;      // Pivot Lookback
...
//+------------------------------------------------------------------+
//| Helper: VWAP proxy (use EMA21 as VWAP proxy in MT5)             |
//+------------------------------------------------------------------+
...
void CalcScores(double &bullScore, double &bearScore) {
   ...
   // === LIQUIDITY SWEEP + BOS ===
   bool liqSweepBull = lastSwingLow > 0 && l1 < lastSwingLow && c1 > lastSwingLow && bosUp;
   bool liqSweepBear = lastSwingHigh > 0 && h1 > lastSwingHigh && c1 < lastSwingHigh && bosDown;

   // === OTE FIBO ===
   bool oteBull = false, oteBear = false;
   double oteZoneBullTop = 0, oteZoneBullBot = 0;
   double oteZoneBearTop = 0, oteZoneBearBot = 0;
   if(lastSwingHigh > 0 && lastSwingLow > 0 && lastSwingHigh > lastSwingLow) {
      double rng = lastSwingHigh - lastSwingLow;
      oteZoneBullTop = lastSwingHigh - 0.618 * rng;
      oteZoneBullBot = lastSwingHigh - 0.786 * rng;
      oteZoneBearTop = lastSwingLow  + 0.786 * rng;
      oteZoneBearBot = lastSwingLow  + 0.618 * rng;
   ...
   double range1  = h1 - l1;
   bool displacementUp   = eqLow  > 0 && l1 < eqLow  && c1 > eqLow  && (c1 - o1) > range1 * 0.6;
   bool displacementDown = eqHigh > 0 && h1 > eqHigh && c1 < eqHigh && (o1 - c1) > range1 * 0.6;
   ...
   bool imbalanceBull = delta > deltaAvg * 1.5 && vol1 > avgVol * 1.8;
   bool imbalanceBear = delta < deltaAvg * -1.5 && vol1 > avgVol * 1.8;
   ...
   double stochK1 = GetStochK(1);
   double stochK2 = GetStochK(i_pivLen * 2 + 1);
   ...
   bbWidthAvg /= 20.0;
   bool bbSqueeze    = bbWidth < bbWidthAvg * 0.7;
   bool bbSqueeze2   = false;
   double u2, l2b, b2;
   GetBB(u2, l2b, b2, 2);
   bbSqueeze2 = (u2 - l2b) < bbWidthAvg * 0.7;
   bool bbBreakoutUp = bbSqueeze2 && c1 > bbUpper && emaRibbonBull;
   bool bbBreakoutDn = bbSqueeze2 && c1 < bbLower && emaRibbonBear;

   // === SUPERTREND ===
   int stDir1, stDir2;
   double st1 = GetSupertrend(stDir1, 1);
   double st2 = GetSupertrend(stDir2, 2);
   ...
   double ichiConvPrev = 0, ichiBasePrev = 0;
   ...
   ichiConvPrev = (cH2 + cL2) / 2.0;
   ichiBasePrev = (bH2 + bL2) / 2.0;
   ...
   // === GOD-TIER HYBRIDS ===
   bool wyckoffSpring   = l1 < lastSwingLow  && c1 > lastSwingLow  && bullOBRetest && posVolDeltaAtZone;
   bool wyckoffTerminal = h1 > lastSwingHigh && c1 < lastSwingHigh && bearOBRetest && negVolDeltaAtZone;
...
void PlaceTrade(bool isBuy) {
   ...
   if(isBuy)
      trade.Buy(lots, Symbol(), price, sl, tp1, "GOD MODE BUY");
   else
      trade.Sell(lots, Symbol(), price, sl, tp1, "GOD MODE SELL");
}
...
int OnInit() {
   ...
   int macdHandleTemp = iMACD(Symbol(), PERIOD_CURRENT, 12, 26, 9, PRICE_CLOSE);
   macdHandle = macdHandleTemp;
   ...
   Print("ULTIMATE GOD MODE EA v6 initialized on ", Symbol());
...
void OnTick() {
   ...
   // Logging
   if(godBuy)
      Print("🌟 GOD MODE BUY | Score: ", finalBullScore, "/100 | ", Symbol(), " | ", TimeToString(TimeCurrent()));
   if(godSell)
      Print("🌟 GOD MODE SELL | Score: ", finalBearScore, "/100 | ", Symbol(), " | ", TimeToString(TimeCurrent()));
