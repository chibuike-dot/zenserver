//+------------------------------------------------------------------+
//|  ULTIMATE v8 GOD MODE EA — ICT/SMC Confluence                   |
...
//+------------------------------------------------------------------+
#property copyright "ULTIMATE GOD MODE EA v8"
...
void CalcScores(double &bullScore, double &bearScore) {
   ...
   double eqL= MathAbs(l2-iLow(Symbol(), PERIOD_CURRENT,4))<ts*5 ? l2 : 0;
   double rng1=h1-l1;
   bool dispUp  = eqL>0 && l1<eqL && c1>eqL && rng1>0 && (c1-o1)>rng1*0.6;
   bool dispDown= eqH>0 && h1>eqH && c1<eqH && rng1>0 && (o1-c1)>rng1*0.6;
   ...
   for(int i=1;i<=14;i++) {
      double v=(double)iVolume(Symbol(),PERIOD_CURRENT,i);
      double c=iClose(Symbol(),PERIOD_CURRENT,i), o=iOpen(Symbol(),PERIOD_CURRENT,i);
      double h=iHigh(Symbol(),PERIOD_CURRENT,i),  lo=iLow(Symbol(),PERIOD_CURRENT,i);
      double r=h-lo; if(r==0) continue;
      double bv=c>o?v:v*((c-lo)/r);
      double d=bv-(v-bv);
      if(i==1) delta=d;
      deltaAvg+=d;
   }
   ...
   for(int i=1;i<=20;i++){double u,l,m; GetBB(u,l,m,i); bbwAvg+=u-l;} bbwAvg/=20.0;
   double u2,l2b,m2; GetBB(u2,l2b,m2,2);
   bool sqz2=(u2-l2b)<bbwAvg*0.7;
   ...
   double rng1=h1-l1, body1=MathAbs(c1-o1);
   double uwk=h1-MathMax(c1,o1), lwk=MathMin(c1,o1)-l1;
   bool isHmr = rng1>0&&lwk>=body1*2&&uwk<body1*0.5&&c1>o1;
   bool isSS  = rng1>0&&uwk>=body1*2&&lwk<body1*0.5&&c1<o1;
   ...
   bool isDoji   = rng1>0&&body1<rng1*0.15;
...
void PlaceTrade(bool isBuy) {
   ...
   if(isBuy) trade.Buy(lots,Symbol(),price,sl,tp,"GOD v8 BUY");
   else      trade.Sell(lots,Symbol(),price,sl,tp,"GOD v8 SELL");
}
...
int OnInit() {
   ...
   Print("ULTIMATE GOD MODE EA v8 initialized | Magic:",magicNumber,
